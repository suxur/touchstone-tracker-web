...
import { Combatant, VoidFn } from '@/types';
...
export const MoveableStatBlock = ({ combatant, close }: Props) => {
  ...
  return (
    <>
      <div
        ref={target}
        className={clsx('stat-block cursor-move border-none absolute top-1/4 ml-auto mr-auto left-0 right-0 min-w-64 min-h-24', {'opacity-75 h-24 w-64': collapsed, 'w-96 h-1/2': !collapsed})}
      >
        <StatBlockQuery ref={button} id={combatant.id} type="monster" />
      </div>
      <Moveable
        bounds={{
          top: 10,
