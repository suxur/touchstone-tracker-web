...
export const Dropdown = forwardRef<HTMLDivElement, Props>(
  ({ value, label, onChange, data, placeholder, required = false }, ref) => (
    <Listbox as="div" className="w-full" value={value} onChange={(e) => onChange(e)} ref={ref}>
      {label && (
        <JetLabel
          htmlFor="class"
          value={label}
      ...
      <div className="relative flex flex-grow">
        <Listbox.Button className="h-10 w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:border-purple-300 focus:ring focus:ring-purple-200 focus:ring-opacity-50 sm:text-sm">
          <span className="block text-base truncate">
            {value ? startCase(value) : placeholder || `Select a ${label}`}
          </span>
